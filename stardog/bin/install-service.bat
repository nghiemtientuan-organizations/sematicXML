...
set CLASSPATH=%STARDOG_INSTALL%\client\ext\*;%STARDOG_INSTALL%\client\api\*;%STARDOG_INSTALL%\client\cli\*;%STARDOG_INSTALL%\client\http\*;%STARDOG_INSTALL%\client\pack\*;%STARDOG_INSTALL%\server\dbms\*;%STARDOG_INSTALL%\server\ext\*

stardog-server //IS//Stardog-Server --Classpath=%CLASSPATH% --DisplayName="%STARDOG_SERVICE_DISPLAY_NAME%" --Install=%STARDOG_INSTALL%\bin\Stardog-Server.exe --LogPath=%STARDOG_LOG_PATH% --StdOutput=auto --StdError=auto --StartMode=jvm --StopMode=exe --StartClass=com.complexible.stardog.cli.admin.CLI --StartParams=server;start --StopClass=com.complexible.stardog.cli.admin.CLI --StopImage=%STARDOG_INSTALL%\bin\stop-service.bat --JvmOptions=-Xrs;-Dstardog.install.location=%STARDOG_INSTALL%;-Djava.library.path=%STARDOG_INSTALL%\lib --JvmMx=%STARDOG_MEMORY% --JvmMs=%STARDOG_MEMORY% ++Environment=STARDOG_HOME=%STARDOG_HOME%
