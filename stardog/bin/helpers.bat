...
SET DEFAULT_JAVA_ARGS=-Djavax.xml.datatype.DatatypeFactory=org.apache.xerces.jaxp.datatype.DatatypeFactoryImpl -Dapple.awt.UIElement=true -Dfile.encoding=UTF-8 -Djdk.tls.client.protocols=TLSv1.2
...
IF NOT DEFINED STARDOG_JAVA_ARGS (
    ...
    IF NOT DEFINED STARDOG_SERVER_JAVA_ARGS  (
        SET STARDOG_SERVER_JAVA_ARGS=-Xmx2g -Xms2g -XX:MaxDirectMemorySize=1g
    )
) ELSE (
    IF NOT DEFINED STARDOG_SERVER_JAVA_ARGS (
        SET STARDOG_SERVER_JAVA_ARGS=%STARDOG_JAVA_ARGS%
    )
)
...
SET STARDOG_SERVER_JAVA_ARGS=-Dstardog.home=%STARDOG_HOME% %STARDOG_SERVER_JAVA_ARGS%
...
IF DEFINED APP_START (
    ...
    IF NOT DEFINED log4j_location (
        SET log4j_location="%STARDOG%\server\dbms\log4j2.xml"
    )
) ELSE (
    SET log4j_location="%STARDOG%\client\cli\log4j2.xml"
)
